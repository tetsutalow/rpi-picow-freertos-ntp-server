...
4. IPアドレスの設定をします。DHCPでもStaticでも可能です。接続後IPアドレスが表示されます。
...
2026/3/15 v0.2 GPSに同期していなくともRTCの時刻を返すようにした(但しstratumは16とする) LEDの点滅を内部動作が表現できるように少し変更した。
2026/3/30 v0.3 GPSのNMEAを解析して時刻を補正するようにした(但しstratumは16とする) Static IPが設定できるようにした。
